...
**Why it's here.** An earlier version squeezed capacity planning into a few input boxes and returned a confident AP count. That math was removed: no single formula can honor the variables that actually drive capacity, and a confidently wrong number is worse than no number. The tile, the `/tools/capacity-planner` route, and the "Capacity Planner" title are kept so it still resolves where users expect. It now tells the truth instead of computing false precision.

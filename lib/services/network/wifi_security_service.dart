...
class WifiSecurityInfo {
  ...
  /// Whether Location Services is currently authorized (the shared gate).
  final bool locationAuthorized;

  /// An unavailable result with a [reason]. [locationAuthorized] defaults false.
  const WifiSecurityInfo.unavailable(this.reason, {this.locationAuthorized = false})
      : available = false,
        securityToken = null,
        bssid = null,
        ssid = null;
...
class WifiSecurityService {
  ...
  Future<WifiSecurityInfo> fetch() async {
    try {
      ...
      return WifiSecurityInfo.unavailable(e.message ?? 'Channel error.');
    }
  }

...
class PiBackendQualityClient implements QualityClient {
  ...
  /// A DECLINED metric is not a failed one. When the caller opts a metric out,
  /// the note names the CHOICE and its reason rather than implying the Pi tried
  /// and could not, which would be a claim about the network that nothing
  /// measured (GL-005).
  static const String _skippedThroughput =
      'Not requested for this run, so no throughput was measured and none is '
      'claimed.';
  static const String _skippedResponsiveness =
      'Not requested for this run, so no responsiveness figure was measured and '
      'none is claimed.';

  QualityResult _resultFrom(
    PiConntestResult ct,
    PiThroughputResult? tp, {
    required bool includeThroughput,
    required bool includeResponsiveness,
